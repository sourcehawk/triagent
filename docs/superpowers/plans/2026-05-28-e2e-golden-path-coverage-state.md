...
| sourcehawk/triagent#18    | e2e-flows34-editors          | .claude/worktrees/e2e-golden-path-coverage--editors      | → feature/e2e-golden-path-coverage | dispatched  |
| sourcehawk/triagent#19    | e2e-flow5-repos              | .claude/worktrees/e2e-golden-path-coverage--repos        | → feature/e2e-golden-path-coverage | dispatched  |
...
- **2026-05-28 — wave-2 checkpoint CLEAN; wave 3 dispatched (#18/#19).** `reviewing-feature-progress` on feature tip `4a5be79`: acceptance criteria met (#15 formal criteria green — Approach's `--cwd`/`--launch-browser` are spec-vs-product divergence to surface at integration, `TRIAGENT_PROFILE` precedence covered by unit test; #16 four proposals backend+browser; #17 round-trips via real `list_resources{kind:Pod}` not the issue's `list_pods` shorthand). State-file integrity verified (PRs #20-24 merged, all contracts locked, branch+worktree present). End-to-end on the feature worktree: `make test` EXIT 0 (all Go race pkgs ok + frontend vitest 191/191), `make lint` 0 issues, `npm run typecheck` clean, `make test-e2e` EXIT 0. Dispatched #18 (`e2e-flows34-editors`, editors playbook+wiki) and #19 (`e2e-flow5-repos`, repos page) off `feature/...`. Dispatch prompts carry: locked `browser-harness` (`(*Harness).Browser.Run(t,spec)` + `SetEnv`; `Options.Browser:true`; helpers in `e2e/browser/helpers/triagent.ts`; testid `data-testid="triagent-<component>-<role>"`; env `TRIAGENT_BASE_URL`/`TRIAGENT_TOKEN`/extras); the locked stub vocabulary incl. #16's `proposal`/`record_prompt`/`resume.jsonl` and #17's real `expect_tool_result`; and the **#24 build rule — `make test-e2e` builds the frontend first; any new browser spec MUST add its testids to the real frontend components, and `Browser.Run` now hard-fails if the embedded SPA is missing**.
