...
- **Scope validation.** Any `--project` / `--account` / region in the argv must be in the profile's scope allowlist, so the agent cannot pivot to an un-allowlisted target.
