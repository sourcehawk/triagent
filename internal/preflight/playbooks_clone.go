...
// PlaybooksCloneOpts configures the boot-time upstream playbooks clone.
//
...
type PlaybooksCloneOpts struct {
	Repo    string
	Dir     string
	Offline bool
}

// EnsurePlaybooksClone idempotently makes sure the upstream playbooks
...
//     operator triggers sync explicitly via the playbooks tab.
...
func EnsurePlaybooksClone(ctx context.Context, opts PlaybooksCloneOpts) error {
	if opts.Dir == "" {
		return fmt.Errorf("playbooks dir is empty")
	}
	if err := os.MkdirAll(filepath.Dir(opts.Dir), 0o700); err != nil {
		return fmt.Errorf("create parent of %s: %w", opts.Dir, err)
	}
	exists, gitInside, err := dirState(opts.Dir)
	if err != nil {
		return err
	}
	if exists && gitInside {
	...
	if opts.Repo == "" {
		...
		// downstream commits land somewhere. The push surfaces handle
		// the empty-repo case separately (no remote = no PR creation).
		return initEmptyLocal(ctx, opts.Dir)
	}
	if opts.Offline {
		return fmt.Errorf("playbooks dir %s is empty and the profile is set to offline; populate it manually (e.g. `git clone https://github.com/%s.git %s`) or set defaults.offline=false in the profile", opts.Dir, opts.Repo, opts.Dir)
	}
	...
	if err != nil {
		return fmt.Errorf("clone upstream playbooks repo %s into %s: %w\ngit output:\n%s", cloneURL, opts.Dir, err, strings.TrimSpace(string(out)))
	}
	return nil
}

// dirState reports whether the dir exists, and whether it looks like
...
func dirState(dir string) (exists, isGitCheckout bool, err error) {
