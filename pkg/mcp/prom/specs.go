...
func ToolSpecs() []toolspec.ToolSpec {
	return []toolspec.ToolSpec{
		...
		{Server: "triagent-prom", Name: "prom_query", Description: "Instant PromQL. Scope high-cardinality metrics with a label matcher OR a topk/bottomk/limitk wrapper. Default 50-series cap; pass max_series (up to 500) for a deliberate fleet sweep. Empty result is samples:[], not an error.", Inputs: toolspec.FromStruct(promQueryIn{})},
