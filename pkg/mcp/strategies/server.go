...
type summarizeIn struct {
	...
	Symptom    string `json:"symptom" jsonschema:"Slack-shareable TL;DR of the user-facing symptom — what the operator brought you, normalised. TWO SENTENCES MAX, simple past, active voice, each sentence under 25 words, NO bullets, NO log-line citations, NO timestamps (those belong in evidence). e.g. 'ZeebeClusterUnhealthy on prod-gke-us-east1-worker-2 for ZeebeCluster <id>. CR Ready=False naming elasticsearch + Operate/Tasklist/Optimize webapps; brokers and gateway remained Available.'"`
	RootCause  string `json:"root_cause" jsonschema:"Slack-shareable TL;DR of the likely root cause as plain prose. Name the offending component / commit / change. TWO TO THREE SENTENCES MAX, simple past, active voice, each sentence under 25 words, NO bullets, NO log-line citations, NO embedded timestamps — bullets, log lines, file:line, sha, and condition reasons all belong in evidence. e.g. 'Parameter swap in PortForwardService introduced by example-service commit 3c602a58 (PR #4525): the rebalance subcommand passes (namespace, serviceName) matching the Forwarder alias, but PortForwardService parameters are reversed — port-forward never binds, the POST hangs, and the 30s client timeout fires.'"`
