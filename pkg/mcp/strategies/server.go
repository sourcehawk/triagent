...
type summarizeIn struct {
	...
	RootCause  string `json:"root_cause" jsonschema:"Slack-shareable TL;DR of the likely root cause as plain prose. Name the offending component / commit / change. TWO TO THREE SENTENCES MAX, simple past, active voice, each sentence under 25 words, NO bullets, NO log-line citations, NO embedded timestamps — bullets, log lines, file:line, sha, and condition reasons all belong in evidence. e.g. 'Commit 3c602a58 in example-service (PR #4525) swapped the parameters of PortForwardService. The rebalance subcommand passes (namespace, serviceName) in the Forwarder order, so the port-forward never binds. The POST hangs and the 30s client timeout fires.'"`
